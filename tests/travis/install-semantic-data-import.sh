...
function installToMediaWikiRoot {
	...
	if [ "$SFS" != "" ]
	then
		composer require 'mediawiki/semantic-data-import='$SMT --prefer-source --update-with-dependencies
	else
		composer init --stability dev
		composer require "mediawiki/semantic-data-import:dev-master" --prefer-source --dev --update-with-dependencies

		cd extensions
		cd SemanticDataImport
...
function updateConfiguration {
	...
	echo 'require_once "$IP/extensions/SemanticDataImport/SemanticDataImport.php";' >> LocalSettings.php
